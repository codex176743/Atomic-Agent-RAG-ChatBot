...
from config import CHUNK_SIZE, CHUNK_OVERLAP, NUM_CHUNKS_TO_RETRIEVE, CHROMA_PERSIST_DIR
...
def initialize_system() -> tuple[ChromaDBService, RAGContextProvider]:
    ...
    try:
        ...
        console.print(
            "[dim]• Adding document chunks to vector database...[/dim]")
        chunk_ids = chroma_db.add_documents(
            documents=chunks, metadatas=[
                {"source": "state_of_union", "chunk_index": i} for i in range(len(chunks))]
        )
        console.print(
            f"[dim]• Added {len(chunk_ids)} chunks to vector database[/dim]")
        ...
        console.print(
            "[dim]• Registering context provider with agents...[/dim]")
...
def display_welcome() -> None:
    ...
    welcome_panel = Panel(
        WELCOME_MESSAGE, title="[bold blue]RAG Chatbot[/bold blue]", border_style="blue", padding=(1, 2))
    console.print("\n")
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    "[cyan]Generating semantic search query...", total=None)
                query_output = query_agent.run(
                    RAGQueryAgentInputSchema(user_message=user_message))
                ...
                task = progress.add_task(
                    "[cyan]Searching knowledge base...", total=None)
                search_results = chroma_db.query(
                    query_text=query_output.query, n_results=NUM_CHUNKS_TO_RETRIEVE)

                # Update context with retrieved chunks
                rag_context.chunks = [
                    ChunkItem(content=doc, metadata={
                              "chunk_id": id, "distance": dist})
                ...
                # Generate answer
                task = progress.add_task(
                    "[cyan]Analyzing chunks and generating answer...", total=None)
                qa_output = qa_agent.run(
                    RAGQuestionAnsweringAgentInputSchema(question=user_message))
                progress.remove_task(task)
            ...
            console.print(
                "[dim]Please try again or type 'exit' to quit.[/dim]")
